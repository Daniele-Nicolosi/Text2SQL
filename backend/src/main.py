...
def search_with_retry(body: SearchInput) -> SearchWithRetryOutput:
    ...
    if valid != "valid":
        ...
        return SearchWithRetryOutput(attempt_1=attempt_1, attempt_2=SearchOutput(sql=sql_retry, sql_validation=valid, results=result))

    return SearchWithRetryOutput(attempt_1=SearchOutput(sql=sql_query, sql_validation=valid, results=result), attempt_2=None)
